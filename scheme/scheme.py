...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #operator.scheme_apply(self, operands.map(lambda operands: scheme_eval(ops, env)), env)

        #return operator.eval_call(rest, env)

        #operands = scheme_eval(rest, env)
        #return scheme_apply(operator, operands, env)
        #return scheme_eval(rest, env)

        #def eval_call(self, operands, env):
        #short_eval = lambda ops: scheme_eval(ops, env)
        #return scheme_apply(self, operands.map(short_eval), env)

        #operator = scheme_eval(first, env)
        #print('test0')
        #print(operator)
        #print('test1')
        #print(check_procedure(operator))
        #check_procedure(operator)
        #print('test2', rest, env)
        #operands = scheme_eval(rest, env)
        #prnt('test3')
        #print(operands)
        #print('test4')
        #return scheme_apply(operator, operands, env)        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    #return scheme_eval(expressions.first, env)    # END PROBLEM 8
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('formals length must match values length')
        child = Frame(self)
        formalslen = len(formals)
        for i in range(formalslen):
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        symbol_value = expressions.second.first
        evaluated = scheme_eval(symbol_value, env)
        ...
        #print('test1')
        inputs = target.second
        #print(inputs)
        evaluated = expressions.second
        #print(evaluated)
        #print('test2')
        allofit = LambdaProcedure(inputs, evaluated, env)
        env.define(function_name, allofit)
        #print('test4')
