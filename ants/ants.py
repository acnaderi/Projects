...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt will do nothing each turn, but will serve as a wall."""
    implemented = True
    name = 'Wall'
    food_cost = 4
            ...
            while self.place != None:
                if self.place.ant != None:
                    if self.place.ant.container:
                        if self.place.ant.ant != None:
                            damage_buff = self.place.ant.ant
                            if damage_buff not in QueenAnt.buffed_ants and damage_buff.name is not 'Queen':
                                damage_buff.damage *= 2
                                QueenAnt.buffed_ants.append(damage_buff)
                    damage_buff = self.place.ant
                    if damage_buff not in QueenAnt.buffed_ants:
                        if damage_buff.name != 'Queen':
                            damage_buff.damage *= 2
                            QueenAnt.buffed_ants.append(damage_buff)
                damage_buff = damage_buff.exit
